...
export default function CompaniesIndexPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setCheckError(null);
    try {
      const res = await fetch(`http://localhost:8000/api/market/ticker/${sym}`);
      if (!res.ok) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      <header>
        ...
        <h1 style={{ fontSize: "26px", fontWeight: "700", marginTop: "6px" }}>
          Search a company
        </h1>
        <p style={{ color: "var(--text-muted)", marginTop: "6px", fontSize: "13px", maxWidth: "680px" }}>
          Enter any ticker symbol to view provider-backed profile, financials, valuation metrics,
          and the market chart. Data is sourced live from Finnhub and Coinbase — no fabricated values.
        </p>
      </header>

      {/* Search bar */}
      <section className="card">
        <form onSubmit={handleSearch} style={{ display: "flex", gap: "10px", alignItems: "flex-start" }}>
          <div style={{ flex: 1 }}>
            <input
              type="text"
              value={input}
              onChange={(e) => { setInput(e.target.value.toUpperCase()); setCheckError(null); }}
        ...
        <p style={{ color: "var(--text-muted)", fontSize: "11px", marginTop: "10px" }}>
          Validates against Finnhub before navigating. Any symbol supported by the free Finnhub plan works.
        </p>
      </section>
      ...
      <section className="card" style={{ borderColor: "var(--border-color)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", fontSize: "12px" }}>
          <div>
            ...
            <div style={{ color: "var(--text-muted)", marginTop: "3px" }}>Finnhub (equities) · Coinbase (crypto)</div>
          </div>
          <div>
            <div style={{ fontWeight: "600", color: "var(--text-secondary)" }}>Company profile</div>
            <div style={{ color: "var(--text-muted)", marginTop: "3px" }}>Finnhub stock profile API</div>
          </div>
          <div>
            <div style={{ fontWeight: "600", color: "var(--text-secondary)" }}>Unavailable fields</div>
            <div style={{ color: "var(--text-muted)", marginTop: "3px" }}>Shown as "Unavailable" — never fabricated</div>
          </div>
        </div>
      </section>
    </div>
