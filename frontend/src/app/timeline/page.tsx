      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: "24px" }}>
        ...
        <div className="table-container">
          <div className="table-header">
            ...
            <button className="btn btn-secondary" onClick={fetchEvents} style={{ padding: "4px 8px", fontSize: "11px" }}>
              Refresh
            </button>
          </div>
          <table className="data-table">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={4} style={{ textAlign: "center", padding: "30px" }}>Loading event log...</td>
                </tr>
              ) : events.length === 0 ? (
                <tr>
                  <td colSpan={4} style={{ textAlign: "center", padding: "30px" }}>No events recorded.</td>
                </tr>
              ) : (
                events.map((evt) => {
                  ...
                  return (
                    ...
                    >
                      <td>
                        <span
                          className={`badge ${
                            evt.event_type.includes("Success") || evt.event_type.includes("Completed")
                              ? "badge-green"
                              : evt.event_type.includes("Failed") || evt.event_type.includes("Error")
                              ? "badge-red"
                      ...
                      <td style={{ fontSize: "12px", color: "var(--text-primary)" }}>{evt.source}</td>
                      <td>
          ...
          {selectedEvent ? (
            <div className="card">
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "8px", fontSize: "12px", marginTop: "8px" }}>
                <div><strong>Event ID:</strong> <span className="font-mono" style={{ color: "var(--text-muted)" }}>{selectedEvent.event_id}</span></div>
                <div><strong>Correlation ID:</strong> <span className="font-mono" style={{ color: "var(--text-muted)" }}>{selectedEvent.correlation_id}</span></div>
                <div><strong>Source System:</strong> <span className="font-mono">{selectedEvent.source}</span></div>
                <div><strong>UTC Timestamp:</strong> <span className="font-mono">{selectedEvent.timestamp}</span></div>
              </div>
            ...
            <div className="card" style={{ textAlign: "center", padding: "60px 20px" }}>
              <span style={{ color: "var(--text-muted)", fontSize: "13px" }}>Select an event to inspect its payload</span>
            </div>
          )}
        </div>
      </div>
